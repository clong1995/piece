...
class EBase {
     ...
     * @private
     */
    _batchSetStyle(cssText) {
        this.trim(cssText, {
            char: '}',
            position: 'right'
        }).split('}').forEach(value => this._firstSheet.insertRule(value + '}', this._firstSheet.cssRules.length));
    }
    ...
    formatStyle({
                    fontSize = 16,
                    fontColor = '#353535',
                    fontStyle = 'Hiragino Sans GB, Microsoft YaHei, tahoma, arial, \'\\5b8b\\4f53\', sans-serif',
                    background = '#fafafa',
                    cursor = 'auto',
                    userSelect = 'auto',
                    scrollbarSize = 8,
                    scrollbarRadius = 0,
                    scrollbarColor = '#c0c9cd',
                    scrollbarHoverColor = '#d4d8da',
                    scrollbarBackgroundColor = '#4e4e5a',
                } = {}) {
        this._batchSetStyle(
            ...
            body, button, input, select, textarea, a, li {
                font: ${fontSize}px/1.5 ${fontStyle};
                color: ${fontColor}
            }
